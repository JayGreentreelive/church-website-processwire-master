 ...
 */
 ...
 include(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . "site/templates/includes/search-functions.inc",array('includes'=>true,'namespace'=>true,'modules'=>true,'skipIfNamespace'=>true)));
...
if($q = $sanitizer->selectorValue($input->get->q)) {
 ...
 include(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . "site/templates/includes/search-matches.inc",array('includes'=>true,'namespace'=>true,'modules'=>true,'skipIfNamespace'=>true)));
    ...
    $eventMatches = new \ProcessWire\PageArray();
    $eventMatches->import($matches);

    $mediaMatches = new \ProcessWire\PageArray();
    $mediaMatches->import($matches);

    $otherMatches = new \ProcessWire\PageArray();
